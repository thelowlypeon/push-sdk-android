...
public class VibesTest {
    ...
    public void updateDeviceWithoutExistingCredential() throws Exception {
        ...
        vibes.updateDevice(new TestVibesListener<Credential>() {
            @Override
            public void onFailure(String errorText) {
                actualErrorText = errorText;
            }
    ...
    public void trackMultipleEventTypesSuccess() throws Exception {
        ...
        assertThat(eventStorage.storedEvents().getEvents().size(), is(2));
        assertThat(eventStorage.storedEvents().getEvents().get(0), is(event1));
        assertThat(eventStorage.storedEvents().getEvents().get(1), is(event2));

        waitForRunnables(vibes);
        assertThat(api.results.size(), is(0));
        assertThat(eventStorage.storedEvents().getEvents().size(), is(0));
    }
