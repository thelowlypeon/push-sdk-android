...
public class VibesTest extends TestConfig {
    ...
    @Test
    public void trackInboxMessagesFetchedEventSuccess() throws Exception {
        StubResult<Void> result = StubResult.success(null);
        StubAPI api = new StubAPI(result);
        ...
        assertThat(eventStorage.storedEvents().getEvents().size(), is(1));
        assertThat(eventStorage.storedEvents().getSortedEvents().first().getType(), is(new Event(TrackedEventType.INBOX_FETCH).getType()));
    }
    ...
    @Test
    public void testStoreTrackingDataSuccess() {
        StubResult<Void> result = StubResult.failure(404, "");
        StubAPI api = new StubAPI(result);
        StubAPI trackingApi = new StubAPI(StubResult.failure(200, ""));
        Credential existingCredential = new Credential("existing-device-id", "existing-auth-token");
        StubCredentialManager credentialManager = new StubCredentialManager(existingCredential);
        StubPersistentEventStorage eventStorage = new StubPersistentEventStorage();
        StubPersistentTrackingData trackingStorage = new StubPersistentTrackingData();

        StubVibesReceiver receiver = new StubVibesReceiver();
        receiver.eventStorage = eventStorage;
        receiver.trackingStorage = trackingStorage;
        IntentFilter stubFilter = new IntentFilter(VibesEvent.ACTION_PUSH_OPENED);
        LocalBroadcastManager.getInstance(context).registerReceiver(receiver, stubFilter);
        Vibes vibes = new Vibes(vibesConfig, api, trackingApi, credentialManager, eventStorage, trackingStorage, new StubActivityLifecycleListener(), new NotificationFactory(context));
        Vibes.setInstance(vibes);
        ...
        TrackingData clientTrackingData = vibes.getTrackingData();

        assertThat(clientTrackingData.getActivityType(), is("Broadcast"));
        assertThat(clientTrackingData.getCompanyKey(), is("Vxy1243P"));
        assertThat(clientTrackingData.getPersonId(), is(notNullValue()));
    }

    @Test
    public void testGetTrackingDataWithNoNotification() {
        StubResult<Void> result = StubResult.failure(404, "");
        StubAPI api = new StubAPI(result);
        StubAPI trackingApi = new StubAPI(StubResult.success(null));

        Credential existingCredential = new Credential("existing-device-id", "existing-auth-token");
        StubCredentialManager credentialManager = new StubCredentialManager(existingCredential);
        StubPersistentEventStorage eventStorage = new StubPersistentEventStorage();
        StubPersistentTrackingData trackingData = new StubPersistentTrackingData();

        Vibes vibes = new Vibes(vibesConfig, api, trackingApi, credentialManager, eventStorage, trackingData, new StubActivityLifecycleListener(), new NotificationFactory(context));
        Vibes.setInstance(vibes);

        TrackingData clientTrackingData = vibes.getTrackingData();

        assertThat(clientTrackingData, is(nullValue()));
    }

    @Test
    public void testTrackAdd() {
        StubResult<Void> result = StubResult.failure(400, "");
        StubAPI api = new StubAPI(result);
        StubAPI trackingApi = new StubAPI(StubResult.success(null));
        Credential existingCredential = new Credential("existing-device-id", "existing-auth-token");
        StubCredentialManager credentialManager = new StubCredentialManager(existingCredential);
        ...
        Vibes.getInstance().trackAdd(ProductTest.createProduct(), new TestVibesListener<Void>() {
            @Override
            public void onSuccess(Void value) {
                eventCalled = true;
            }
        ...
        waitForRunnables(vibes);
        assertThat(eventCalled, is(true));
    }
    ...
    public void testTrackClick() {
        ...
        Vibes.getInstance().trackClick(ProductTest.createProduct(), new TestVibesListener<Void>() {
            @Override
            public void onSuccess(Void value) {
                eventCalled = true;
            }
    ...
    public void testTrackDetail() {
        ...
        Vibes.getInstance().trackDetail(ProductTest.createProduct(), new TestVibesListener<Void>() {
            @Override
            public void onSuccess(Void value) {
                eventCalled = true;
            }
    ...
    public void testTrackPurchase() {
        ...
        Vibes.getInstance().trackPurchase(PurchaseTest.createPurchase(), new TestVibesListener<Void>() {
            @Override
            public void onSuccess(Void value) {
                eventCalled = true;
            }
    ...
    private static HashMap<String, String> createPushPayload() {
        ...
        try {
            ...
            clientAppData.put("activity_uid", UUID.randomUUID().toString());
            clientAppData.put("company_key", "Vxy1243P");
