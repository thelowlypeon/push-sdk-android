...
interface VibesAPIInterface {
     ...
     * @param listener the {@link ResourceListener} to notify about success or failure of the request
     * @param <T> the Type of the {@link HTTPResource} that is being requested, e.g. {@link Credential}
     * @param ignoreBaseUrl enables the submission of a non-vibes URL
     */
    <T> void request(HTTPResource<T> resource, ResourceListener<T> listener, boolean ignoreBaseUrl);
...
class VibesAPI implements VibesAPIInterface {
    ...
    /**
     * Same as {@link #request(HTTPResource, ResourceListener, boolean)} with <code>ignoreBaseUrl</code> set to false
     */
    ...
    public <T> void request(HTTPResource<T> resource, ResourceListener<T> listener, boolean ignoreBaseUrl) {
        client.request(resource, null, listener, ignoreBaseUrl);
    }
