...
class VibesWorkerThread extends HandlerThread {
    ...
    /**
     * Sames as {@link #request(VibesAPIInterface, HTTPResource, ResourceListener, boolean)}, with <code>ignoreBaseUrl</code> set to false
     */
    public <T> void request(final VibesAPIInterface api, final HTTPResource<T> resource, final ResourceListener<T> listener) {
        this.request(api, resource, listener, false);
    }
     ...
     */
    public <T> void request(final VibesAPIInterface api, final HTTPResource<T> resource, final ResourceListener<T> listener, final boolean ignoreBaseUrl) {
        this.workerHandler.post(new Runnable() {
            @Override
            public void run() {
                api.request(resource, listener, ignoreBaseUrl);
            }
        });
    }
    ...
    public void onSuccessful(final VibesListener<Person> completion, final Person value) {
        responseHandler.post(new Runnable() {
            @Override
            public void run() {
                completion.onSuccess(value);
