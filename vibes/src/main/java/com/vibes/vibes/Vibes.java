...
public class Vibes implements EventTracker {
     ...
     *
     * TODO: Replace this URL when the production API is available.
     */
     ...
     */
    protected Vibes(String appId, VibesAPIInterface api, CredentialManagerInterface credentialManager, PersistentEventStorageInterface eventStorage, VibesLifecycleListener lifecycleListener) {
        this.appId = appId;
        this.api = api;
    ...
    public void registerDevice(final VibesListener<Credential> completion) {
        ...
        this.workerThread.request(api, resource, listener);
    }
    ...
    public void updateDevice() {
        updateDevice(new EmptyVibesListener<Credential>());
    }
     ...
     */
    public void updateDevice(final VibesListener<Credential> completion) {
        Credential credential = credentialManager.getCurrent();
        if (credential == null) {
        ...
        HTTPResource<Credential> resource = APIDefinition.updateDevice(appId, credential.getDeviceID());
    ...
    public void unregisterDevice(final VibesListener<Void> completion) {
        ...
        retryableRequest(credential, resource, listener, completion);
    }
    ...
    public synchronized void uploadStoredEvents() {
        ArrayList<Event> stored = this.eventStorage.storedEvents().getEvents();
        if (stored.isEmpty()) {
        ...
        if (credential != null) {
            final ResourceListener<Void> listener = new ResourceListener<Void>() {
                ...
                public void onSuccess(Void value) {
                    ...
                    eventStorage.removeEvents(upload);
                    uploadStoredEvents();
                }
            ...
            retryableRequest(credential, resource, listener, new EmptyVibesListener<Void>());
        } else {
     ...
     */
    private <T> void retryableRequest(final Credential credential, final HTTPResource<T> resource, final ResourceListener<T> listener, final VibesListener<T> completion) {
