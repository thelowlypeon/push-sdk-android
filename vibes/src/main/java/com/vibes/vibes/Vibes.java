...
public class Vibes implements EventTracker {
    ...
    public void handleNotification(Context context, Map<String, String> payload) {
        String pushToken = payload.get("vibes_auto_register_token");
        if (pushToken != null && !pushToken.trim().isEmpty()) {
            handleRegistration(pushToken);
        } else {
     ...
     */
    private void handleRegistration(final String token) {
        this.registerDevice(new VibesListener<Credential>() {
            @Override
            public void onSuccess(Credential value) {
                Vibes.getInstance().registerPush(token);
                // TODO Implement registerPush with VibesListener to track logs on
     ...
     * @param completion
     */
    public void fetchInboxMessage(final String messageUid, final VibesListener<InboxMessage> completion) {
        getPerson(new VibesListener<Person>() {
            @Override
            public void onSuccess(Person personValue) {
                Credential credential = credentialManager.getCurrent();
     ...
     * @param completion - a completion for being notified of the result of the update
     */
    public void markInboxMessageAsRead(final String messageId, final VibesListener<InboxMessage> completion) {
     ...
     * @param completion
     */
    public void expireInboxMessage(final String messageId, final Date expirationDate, final VibesListener<InboxMessage> completion) {
        final Credential credential = credentialManager.getCurrent();
        final Person person = personManager.getCurrent();
        if (credential == null) {
            completion.onFailure(NO_CREDENTIALS);
     ...
     * @param completion a completion for being notified of the result of the update
     */
    public void expireInboxMessage(String messageId, final VibesListener<InboxMessage> completion) {
