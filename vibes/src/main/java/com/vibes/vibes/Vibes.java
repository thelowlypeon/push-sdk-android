...
public class Vibes implements EventTracker {
     ...
     *
     * @param updateCredentials a boolean indicating if it's a token update or device info update. Specify false if not certain
     ...
     */
    public void updateDevice(final boolean updateCredentials, Double latitude, Double longitude, final VibesListener<Credential> completion) {
        ...
        HTTPResource<Credential> resource = APIDefinition.updateDevice(vibesConfig.getAppId(), credential.getDeviceID(), updateCredentials);
        if (updateCredentials) {
        ...
        ResourceListener<T> initialListener = new ResourceListener<T>() {
            ...
            public void onFailure(int statusCode, String errorText) {
                if (statusCode == HttpURLConnection.HTTP_UNAUTHORIZED) {
                    updateDevice(false, 0.0, 0.0, new VibesListener<Credential>() {
                        @Override
                        public void onSuccess(Credential value) {
