...
public class Vibes implements EventTracker {
    ...
    public static void initialize(Context context, VibesConfig vibesConfig) {
        ...
        if (!savedAppId.equals(vibesConfig.getAppId())) {
            ...
            LogObject logObject = new LogObject(VibesLogger.Level.WARN, "AppId has changed to: " + vibesConfig.getAppId());
            getCurrentLogger().log(logObject);
        }
    }
    ...
    public synchronized void trackEvents(List<Event> events) {
        ...
        if (event != null && event.getType().equals(TrackedEventType.LAUNCH)) {
            ...
            } else if (!mostRecent.getTimestampSecs().equals(event.getTimestampSecs())) {
    ...
    private void handleRegistration(final String token) {
        this.registerDevice(new VibesListener<Credential>() {
            ...
            public void onSuccess(Credential value) {
                ...
                LogObject logObject = new LogObject(VibesLogger.Level.INFO, "Silent push register success");
                getCurrentLogger().log(logObject);
            }

            @Override
            public void onFailure(String errorText) {
                LogObject logObject = new LogObject(VibesLogger.Level.ERROR, "Failed to register device: " + errorText );
                getCurrentLogger().log(logObject);
            }
        });
    }
