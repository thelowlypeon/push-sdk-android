...
public class PushPayloadParser {
    ...
    public static final String kClientAppData = "client_app_data";
    ...
    public Uri getDeeplinkUri() {
        String deeplLink = getDeepLink();
        if (deeplLink != null && !deeplLink.isEmpty()) {
            try {
                return Uri.parse(deeplLink);
            } catch (Exception ex) {
                Vibes.getCurrentLogger().log(new LogObject(VibesLogger.Level.WARN, "Deeplink supplied=" + deeplLink + " not a valid uri"));
            }
        }
        return null;
    }
