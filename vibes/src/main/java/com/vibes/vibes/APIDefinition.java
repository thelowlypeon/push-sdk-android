...
class APIDefinition {
     ...
     * @param updateCredential if true -> `renew_token`, otherwise `update device Info`
     */
    public static HTTPResource<Credential> updateDevice(String appId, String deviceID, Boolean updateCredential) {
        String path = devicePath(appId, deviceID);
        if (!updateCredential) {
            return new HTTPResource<Credential>(
                    // In case of updating the device only, it won't return any credential.
                    path, PATCH, getEncodedDevice(), jsonHeaders(), emptyCredentialParser());
        } else {
            return new HTTPResource<Credential>(
                    // In case of updating the credentials, a new credential will be returned with the call.
    ...
    static HTTPResource<Void> trackEvents(String appKey, String deviceID, EventCollection collection) {
        ...
        String type = collection.getSortedEvents().first().getType().name().toLowerCase(Locale.ENGLISH);
        String json = null;
        try {
    ...
    /**
     * Triggers silent push migration callback
     *
     * @param migrationItemId migration item Id
     * @param vibesDeviceId   device id
     * @param appId           appId
     * @return HTTPResource
     */
    public static HTTPResource<Void> migrationCallback(String migrationItemId, String vibesDeviceId, String appId) {
        String path = "/" + appId + "/migrations/callbacks";
        JSONObject payload = new JSONObject();
        try {
            payload.put("migration_item_id", migrationItemId);
            payload.put("vibes_device_id", vibesDeviceId);
        } catch (JSONException e) {
            Vibes.getCurrentLogger().log(e);
            /* catch json exception here */
        }
        return new HTTPResource<>(
                path, PUT, payload.toString(), jsonHeaders(), nullParser());
    }
     ...
     * @param data        tracking data for making tracking call.
     * @param activityUid optional activityUid in case user wants to override the one stored in local storage.
     * @return HTTPResource
     */
    public static HTTPResource<Void> trackProductAction(Actions.ProductAction action, Product product, TrackingData data, String activityUid) {
        String actualActivityId = null;
        if (activityUid == null || activityUid.isEmpty()) {
            actualActivityId = data.getActivityUid();
        } else {
            actualActivityId = activityUid;
        }
        ...
        try {
            encodedPath = "?d=" + URLEncoder.encode(path, StandardCharsets.UTF_8.toString());
        } catch (UnsupportedEncodingException e) {
            Vibes.getCurrentLogger().log(e);
        }
        return new HTTPResource<>(
                encodedPath, GET, null, jsonHeaders(), nullParser());
    }
     ...
     * @return HTTPResource
     */
    public static HTTPResource<Void> trackPurchaseAction(Actions.PurchaseAction action, Purchase purchase, TrackingData data, String activityUid) {
        String actualActivityId = null;
        if (activityUid == null || activityUid.isEmpty()) {
            actualActivityId = data.getActivityUid();
        } else {
    ...
    private static JSONResourceParser<VibesAppInfo> mobileAppJSONResourceParser() {
        return new JSONResourceParser<VibesAppInfo>() {
            @Override
            public VibesAppInfo parse(String text) throws JSONException {
                JSONObject json = new JSONObject(text);
                return new VibesAppInfo(json);
            }
        };
    }
    ...
    public static HTTPResource<VibesAppInfo> fetchVibesAppInfo(String appId) {
        String path = "/" + appId;
        return new HTTPResource<>(
                path, GET, null, jsonHeaders(), mobileAppJSONResourceParser());
    }
}
